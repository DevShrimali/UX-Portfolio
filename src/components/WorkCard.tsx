...
import { getYtId, type Project } from "@/data/projects";
...
export default function WorkCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[16/10] overflow-hidden rounded-[20px] bg-surface border border-white/5 transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:-translate-y-1.5 group-hover:shadow-[0_12px_40px_rgba(190,242,100,0.08)]">
        ...
        {project.slug === "logo-design-showcase" ? (
          ...
          <img
            ...
            loading="lazy"
            decoding="async"
            className="w-full h-full object-cover opacity-80 transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:scale-[1.03]"
          />
        )}
        ...
        {/* Year badge */}
        {project.year && (
          <span className="absolute top-4 right-4 z-30 text-[10px] font-bold uppercase tracking-[0.1em] text-black bg-accent px-3 py-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            {project.year}
          </span>
        )}
        ...
        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20 flex items-center justify-center pointer-events-none">
          <div className="w-16 h-16 rounded-full bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20 translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-[600ms] ease-[cubic-bezier(0.23,1,0.32,1)] shadow-xl">
            <svg viewBox="0 0 24 24" className="w-5 h-5 fill-none stroke-white stroke-2 group-hover:stroke-accent transition-colors">
      ...
      <div className="flex flex-col px-2">
        <div className="flex items-center gap-4 mb-2">
          <p className="text-[10px] uppercase font-bold tracking-[0.15em] text-accent">
            {project.category}
          </p>
        ...
        <h3 className="text-2xl font-bold tracking-tight text-white mb-2 transition-colors duration-300 group-hover:text-accent">
          {project.title}
        </h3>
